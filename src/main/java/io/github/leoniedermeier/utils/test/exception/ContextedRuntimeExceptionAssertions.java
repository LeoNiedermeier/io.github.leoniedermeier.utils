...
public final class ContextedRuntimeExceptionAssertions {
	...
	/**
	 * Custom assert method checking whether the given {@link ContextedRuntimeException} has the given {@link ErrorCode}.
	 * 
	 * @param expectedErrorCode The expected {@link ErrorCode}.
	...
	public static void assertExceptionHasErrorCode(ErrorCode expectedErrorCode, ContextedRuntimeException exception) {
		Optional<ErrorCode> findAny = exception.getErrorCodes().filter(expectedErrorCode::equalsTo).findAny();
		if (!findAny.isPresent()) {
			List<String> codes = exception.getErrorCodes().map(ErrorCode::code).collect(toList());
			throw new AssertionFailedError(
					"Exception does not contain errorcode " + expectedErrorCode.code() + "\navailable error codes: " + codes,
					expectedErrorCode.code(), codes);
		}
	}
